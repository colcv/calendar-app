...
function printSelectedMonth2(events, calendarID, tasks) {
  getSelectedMonth();
  return printMonth2(currentMonth, currentYear, events, calendarID, tasks);
}
...
function printMonth2(mm, yy, events, calendarID, tasks) {
  var res = '';
  res += printTable2(mm, yy, events, calendarID, tasks);
  return res;
}
...
function printTable2(mm, yy, events, calendarID, tasks) {
  var i, j, k, solar, lunar, cellClass, solarClass, lunarClass;
  var currentMonth = getMonth(mm, yy);
  if (currentMonth.length == 0) return;
  var ld1 = currentMonth[0];
  ...
  for (i = 0; i < 6; i++) {
    ...
    for (j = 0; j < 7; j++) {
      ...
      if (k < emptyCells || k >= emptyCells + currentMonth.length) {
        ...
        solar = k - emptyCells + 1;
        ld1 = currentMonth[k - emptyCells];
        res += printCell2(ld1, solar, mm, yy, i);
        res_1 += printEvent(ld1, solar, mm, yy, i, events, calendarID, tasks);
      }
    }
    // res += ("</div>\n");
  }
...
function getPrevMonthLink(mm, yy) {
  ...
  return (
    ...
    '&mm=' +
    mm1 +
    '"><i class="fa fa-angle-left fa-lg" aria-hidden="true"></i></a>'
  );
}
...
function getPrevYearLink(mm, yy) {
  ...
  return (
    ...
    '&mm=' +
    mm +
    '"><i class="fa fa-angle-double-left fa-lg" aria-hidden="true"></i></a>'
  );
}
...
function printCell2(lunarDate, solarDate, solarMonth, solarYear, row) {
  var cellClass, solarClass, lunarClass, solarColor;
  cellClass = 'day';
  solarClass = 't2t6';
  lunarClass = 'am';
  ...
  var res = '';
  var args = solarYear + '-' + solarMonth + '-' + solarDate;
  res += '<div class="' + cellClass + '" id="' + args + '">' + solarDate;
  ...
  //     if (checkDate(events[i].create, events[i].end, args)) {
  //       var eventColor;
  //       if (events[i].color == "red") {
  //         eventColor = "event--danger";
  //       } else if (events[i].color == "yellow") {
  //         eventColor = "event--warning";
  //       } else {
  //         eventColor = "event--info";
  //       }
  //       res +=
  //         '<div class="event ' +
  //         eventColor +
...
function printEvent(
  ...
  if (events) {
    for (let i = 0; i < events.length; i++) {
      if (checkDate(events[i].start, events[i].end, args)) {
        var eventColor;
        if (events[i].color == 'red') {
          eventColor = 'event--danger';
        } else if (events[i].color == 'yellow') {
        ...
        if (calendarID) {
          res +=
            ...
            ';">' +
            events[i].title +
            `<div class="dropdown-content"><a class="a-1" href="/calendars/${calendarID}/edit-event/${events[i]._id}">Chi tiết/Chỉnh sửa<i class="far fa-edit"></i></a><a class="a-2 delete-event" id="${calendarID}/events/${events[i]._id}">Xóa<i class="far fa-trash-alt"></i></a></div>` +
            '</div>\n';
        } else {
          res +=
            '<div class="event ' +
  ...
  if (tasks) {
    for (let i = 0; i < tasks.length; i++) {
      if (checkDate(tasks[i].start, tasks[i].start, args)) {
        res +=
          '<div class="task ' +
          '" style="grid-column:' +
          dow +
          ...
          ';">&bull; ' +
          tasks[i].title +
          `<div class="dropdown-content"><a class="a-1" href="/calendars/${calendarID}/edit-task/${tasks[i]._id}">Chi tiết/Chỉnh sửa<i class="far fa-edit"></i></a><a class="a-2" id="${calendarID}/tasks/${tasks[i]._id}">Xóa<i class="far fa-trash-alt"></i></a></div>` +
          '</div>\n';
      }
    }
  }
